...
class ProductRemoteDataSource: ProductDataSource, KoinComponent {
    ...
    override suspend fun save(token: String?, product: ProductDTO): ProductDTO {
        ...
        val response = serviceApi?.addProduct(ProductConstant.APPLICATION_ID, ProductConstant.REST_API_KEY, map, raw)

        return ProductDTO(response.objectId, response.name, response.description,
